...
#  Versi: Pengamanan Kredensial dengan st.secrets
# ===================================================================================
...
import numpy as np
import time # Diimpor untuk menambahkan jeda
...
def load_data_from_gsheets():
    ...
    for i, sheet_name in enumerate(sheet_names):
        try:
            ...
            time.sleep(1) # Jeda untuk menghindari error kuota API
    ...
    with col2:
        if st.button('🚀 Tarik Data & Mulai Analisis', use_container_width=True):
            ...
            if df is not None:
                ...
                st.rerun()
    st.info("Klik tombol untuk memulai. Pastikan Anda telah membuat file `.streamlit/secrets.toml` dengan kredensial Google Sheets Anda.")
    st.stop()
...
with tab1:
    ...
    with col2:
        fig_cat = px.bar(
            category_omzet_sorted, x='Omzet', y='Kategori', orientation='h',
            title=f'Top {num_bars} Kategori Berdasarkan Omzet', text='Omzet',
    ...
    with col_brand:
        ...
        fig_brand = px.pie(values=top_brands.values, names=top_brands.index, title=f'Distribusi Omzet Brand di {MY_STORE_NAME}', hole=0.3)
    ...
    with col_wow:
        ...
        weekly_summary = df_filtered[df_filtered['Toko'] == MY_STORE_NAME].groupby('Minggu').agg(Omzet=('Omzet', 'sum'), Penjualan_Unit=('Nama Produk', 'count')).reset_index().sort_values('Minggu')
...
with tab2:
    ...
    selected_product = st.selectbox("Pilih Produk untuk Dibandingkan", options=product_list, index=None, placeholder="Ketik untuk mencari produk...")
    if selected_product:
...
with tab3:
    ...
    if competitor_latest_overall.empty:
        ...
        brand_analysis = competitor_latest_overall.groupby('Brand').agg(Total_Omzet=('Omzet', 'sum'), Unit_Terjual=('Terjual per Bulan', 'sum')).sort_values('Total_Omzet', ascending=False).reset_index()
        col1, col2 = st.columns(2)
        with col1:
        ...
        with col2:
            ...
            fig_brand_comp = px.pie(top_brands_comp, values='Total_Omzet', names='Brand', title='Top 10 Brand Kompetitor Berdasarkan Omzet', hole=0.3)
...
with tab4:
    ...
    fig_stock = px.line(stock_status_weekly, x='Minggu', y='Jumlah Produk', color='Toko', line_dash='Status', title='Perbandingan Jumlah Produk Ready vs Habis per Minggu', markers=True, labels={'Jumlah Produk': 'Jumlah Produk', 'Minggu': 'Minggu'})
...
with tab5:
    ...
    omzet_pivot_formatted = omzet_pivot.applymap(lambda x: f"{x:,.0f}")
    st.dataframe(omzet_pivot_formatted, use_container_width=True)
    st.subheader("Visualisasi Tren Omzet Toko")
    fig_omzet_toko = px.line(omzet_per_toko_daily, x='Tanggal', y='Omzet', color='Toko', title='Tren Omzet Harian Antar Toko', labels={'Omzet': 'Total Omzet (Rp)', 'Tanggal': 'Tanggal'})
    ...
    if len(unique_weeks) < 2:
        ...
        with col2:
            ...
            if not comparison_options:
                st.warning("Tidak ada minggu sebelumnya untuk dijadikan pembanding."); st.stop()
        ...
        if not new_products:
            ...
            new_products_df = latest_entries_overall[(latest_entries_overall['Nama Produk'].isin(new_products)) & (latest_entries_overall['Toko'] == MY_STORE_NAME)].copy()
...
with st.sidebar:
    ...
    st.download_button('📥 Unduh Data (CSV)', data=convert_df_for_download(df_filtered, 'csv'), file_name=f'analisis_{start_date}_{end_date}.csv', mime='text/csv', use_container_width=True)
    st.download_button('📥 Unduh Data (JSON)', data=convert_df_for_download(df_filtered, 'json'), file_name=f'analisis_{start_date}_{end_date}.json', mime='application/json', use_container_width=True)
