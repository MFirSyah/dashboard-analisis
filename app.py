...
def fuzzy_match_kategori(df_db, df_database, threshold=90):
    ...
    for product in db_products:
        try:
            # Menggunakan token_sort_ratio untuk fleksibilitas urutan kata
...
if not st.session_state.analisis_dimulai:
    ...
    with st.sidebar:
        ...
        # --- PERBAIKAN: Default fuzzy match diubah ke 90 ---
        fuzzy_threshold = st.slider("Tingkat Akurasi Fuzzy Match (%)", 50, 100, 90)
        st.caption("Akurasi lebih tinggi = lebih ketat. Turunkan jika banyak produk 'Tidak Terkategori'.")
    ...
    with tab1:
        ...
        if df_db_klik.empty:
            ...
            if 'Omzet' in df_db_klik_kategori.columns:
                ...
                df_display_kategori = produk_per_kategori[['NAMA', 'HARGA', 'Terjual/Bulan', 'Status']].rename(columns={'HARGA': 'HARGA Terakhir'})
                st.dataframe(
                    df_display_kategori,
                    column_config={"HARGA Terakhir": st.column_config.NumberColumn(format="Rp %d")},
                ...
                merged_produk['Indikator'] = ((merged_produk['Omzet'] - merged_produk['Omzet_lalu']) / merged_produk['Omzet_lalu'].replace(0,np.nan)) * 100
        ...
        if not all_products_list:
            ...
            if selected_product:
                ...
                for store in store_names:
                    ...
                    if df_store.empty or 'NAMA' not in df_store.columns: continue

                    match, score = process.extractOne(selected_product, df_store['NAMA'].unique(), scorer=fuzz.token_sort_ratio)
                    if score >= fuzzy_threshold:
                        matched_hist = df_store[df_store['NAMA'] == match].sort_values('TANGGAL')
    ...
    with tab4:
        ...
        if omzet_data:
            ...
            st.dataframe(df_omzet_pivot.sort_index(ascending=False), column_config=format_dict, use_container_width=True)
        ...
        if not all_products_df.empty and 'TANGGAL' in all_products_df.columns:
            ...
            if len(unique_weeks) < 2:
                ...
                if target_week and comparison_week and target_week != comparison_week:
                    produk_target = set(all_products_df[all_products_df['Minggu'] == target_week]['NAMA'].unique())
                    produk_pembanding = set(all_products_df[all_products_df['Minggu'] == comparison_week]['NAMA'].unique())
