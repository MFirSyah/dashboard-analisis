...
@st.cache_data(show_spinner="Memproses semua file yang diunggah...")
def load_and_process_data(uploaded_files):
    if not uploaded_files:
        return pd.DataFrame(), pd.DataFrame(), None
    is_csv = uploaded_files[0].name.lower().endswith('.csv')
    rekap_list_df = []
    ...
    for f in uploaded_files:
        filename = f.name
        try:
            is_database_file = "DATABASE" in filename.upper()
            is_rekap_file = "REKAP" in filename.upper()
            if is_database_file:
                if is_csv: database_df = pd.read_csv(f)
                else: database_df = pd.read_excel(f)
                database_df.columns = [str(col).strip().upper() for col in database_df.columns]
                db_store_name_match = re.match(r"^(.*?) - DATABASE", filename, re.IGNORECASE)
                if db_store_name_match:
                    my_store_name = db_store_name_match.group(1).strip()
            elif is_rekap_file:
                store_name_match = re.match(r"^(.*?) - REKAP", filename, re.IGNORECASE)
                ...
                if is_csv:
                    ...
                    if "READY" in filename.upper(): df_sheet['Status'] = 'Tersedia'
                    elif "HABIS" in filename.upper(): df_sheet['Status'] = 'Habis'
                    rekap_list_df.append(df_sheet)
                else:
                    xls = pd.ExcelFile(f)
                    for sheet in ['READY', 'HABIS']:
                        if sheet in xls.sheet_names:
                            df_sheet = pd.read_excel(f, sheet_name=sheet)
                            df_sheet.columns = [str(col).strip().upper() for col in df_sheet.columns]
                            df_sheet['Toko'] = store_name
                            df_sheet['Status'] = 'Tersedia' if sheet.upper() == 'READY' else 'Habis'
                            rekap_list_df.append(df_sheet)
        except Exception as e:
            st.warning(f"Gagal memproses file {filename}: {e}")
    if not rekap_list_df: return pd.DataFrame(), pd.DataFrame(), None
    ...
    for col in required_cols:
        if col not in rekap_df.columns:
            st.error(f"Kolom krusial '{col}' tidak ditemukan di file REKAP. Pastikan nama kolom sudah benar.")
            return pd.DataFrame(), pd.DataFrame(), None
...
st.sidebar.info("Unggah semua file Anda. Pastikan semua file dalam format yang seragam (.xlsx semua atau .csv semua).")
uploaded_files = st.sidebar.file_uploader("Pilih file .xlsx atau .csv", type=['xlsx', 'csv'], accept_multiple_files=True)

if not uploaded_files:
    st.info("Silakan unggah file Anda pada sidebar untuk memulai analisis.")
    st.stop()

df, db_df, my_store_name_from_db = load_and_process_data(uploaded_files)

if df.empty:
    st.error("Gagal memuat data REKAP. Pastikan format file dan nama kolom sudah benar.")
    st.stop()
if db_df.empty:
    st.warning("File DATABASE tidak ditemukan atau gagal dibaca. Tab 'Analisis Toko Saya' tidak akan tersedia.")
if my_store_name_from_db is None and not db_df.empty:
    st.warning("Nama toko Anda tidak dapat diidentifikasi dari nama file DATABASE. Gunakan format 'NAMA TOKO - DATABASE.[xlsx/csv]'.")

st.sidebar.header("Langkah 2: Filter Data")
